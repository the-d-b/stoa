...
func ListSessions(db *sql.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		switch days {
		case "1":
			timeFilter = "AND s.issued_at >= datetime('now', '-1 day')"
		case "7":
			timeFilter = "AND s.issued_at >= datetime('now', '-7 days')"
		case "30":
			timeFilter = "AND s.issued_at >= datetime('now', '-30 days')"
		}
		...
		`)
		if err != nil {
			logErrorf("SESSIONS", "query error: %v", err)
