/**
 * NewIntegrationForm — shared integration creation form used by both
 * system settings (scope='system') and personal profile (scope='personal').
 */
import { useState } from 'react'
import { integrationsApi, secretsApi, weatherApi, steamApi } from '../../api'
...
interface Props {
  ...
  onCreated: () => void
  onCancel: () => void
  onSecretsChanged: (secrets: any[]) => void  // parent updates its secret list
}

export default function NewIntegrationForm({
  scope, secrets, onCreated, onCancel, onSecretsChanged
}: Props) {
  ...
  const test = async () => {
    ...
    try {
      const res = await integrationsApi.test({ type, apiUrl, secretId: secretId || undefined, skipTls })
  ...
  const create = async () => {
    if (!name || (!apiUrl && !NO_URL_REQUIRED.includes(type))) return
    setCreating(true)
    try {
      await integrationsApi.create({
        name, type, apiUrl, uiUrl,
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      {/* Row 1: Name, Type, Secret — always visible */}
      ...
      {/* Row 2: URL config — varies by type */}
      {type === 'weather' ? (
      ...
      <div style={{ display: 'flex', gap: 12, alignItems: 'center', flexWrap: 'wrap' }}>
        ...
        <div style={{ flex: 1 }} />
        {!NO_TEST_TYPES.includes(type) && (
          <button className="btn btn-secondary" onClick={test} disabled={testing || !apiUrl}>
            {testing ? <span className="spinner" /> : 'Test'}
          </button>
        )}
        <button className="btn btn-primary" onClick={create}
          disabled={creating || !name || (!NO_URL_REQUIRED.includes(type) && !apiUrl)}>
