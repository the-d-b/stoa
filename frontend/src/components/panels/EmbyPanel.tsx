...
interface EmbyLibrary { title: string; type: string; count: number }
...
interface EmbyData {
  ...
  libraries: EmbyLibrary[]; sessions: EmbySession[]
...
const CHEVRON_COLOR = '#00a4dc'  // Emby blue

// Chevron scrubber — concave right-facing shape matching Emby's brand color
    ...
    <div style={{ position: 'relative', height: 14 }}>
      ...
      {p > 0 && (
        <div style={{
          ...
          width: `${p}%`, background: 'rgba(0,164,220,0.28)',
...
export default function EmbyPanel({ panel, heightUnits }: { panel: Panel; heightUnits: number }) {
  const [data, setData] = useState<EmbyData | null>(null)
  ...
  const sseData = useSSE<EmbyData>(integrationId)
  ...
  // ── 4x: same chips in the libraries section ────────────────────────────────
  const LibraryChips = () => (
              ...
              <div style={{ flex: 1, position: 'relative', height: barH }}>
                ...
                <div style={{ position: 'absolute', inset: 0, borderRadius: 4, overflow: 'hidden' }}>
                  ...
                  {pct > 0 && (
                    <div style={{
                      position: 'absolute', top: 0, left: 0, bottom: 0,
                      width: `${pct}%`, background: 'rgba(0,164,220,0.28)',
  ...
  const SessionCard = ({ s }: { s: EmbySession }) => {
