  ...
  return (
    <div style={{ display: 'flex', gap: 14, fontSize: 12, flexShrink: 0 }}>
      ...
      {data.libraries.length > 0 && (
        <span style={{ color: 'var(--text-dim)' }}>
          <strong style={{ color: 'var(--text)' }}>{data.libraries.length}</strong> libraries
        </span>
      )}
      {data.totalFiles > 0 && (
        <span style={{ color: 'var(--text-dim)' }}>
          <strong style={{ color: 'var(--text)' }}>{data.totalFiles.toLocaleString()}</strong> files
        </span>
      )}
    </div>
  )
...
export default function KavitaPanel({ panel, heightUnits }: { panel: Panel; heightUnits: number }) {
  ...
  // ── 4x+: stats + covers + recently added list ─────────────────────────────
  return (
    <div style={{ padding: '10px 14px', height: '100%', overflow: 'hidden',
      display: 'flex', flexDirection: 'column', gap: 8 }}>
      <StatsRow data={data} />
