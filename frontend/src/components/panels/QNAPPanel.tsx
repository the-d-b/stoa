...
interface QNAPVolume { label: string; status: string; raidType: string; fsType: string; totalGb: number; usedGb: number; usedPct: number }
interface QNAPDisk { id: string; model: string; sizeGb: number; tempC: number; status: string; smartStatus: string }
interface QNAPData {
  uiUrl: string; hostname: string; model: string; fwVersion: string; uptimeSecs: number
  cpuPercent: number; ramPercent: number; ramTotalGb: number; ramUsedGb: number
  volumes: QNAPVolume[]; disks: QNAPDisk[]
  netRxMbs: number; netTxMbs: number
  shares: string[]
}
...
function volumeStatusColor(status: string) {
  ...
  if (s === 'normal' || s === 'ready' || s === 'good') return 'var(--green)'
  if (s === 'degraded' || s === 'rebuilding' || s === 'initializing') return 'var(--amber)'
  if (s === 'error' || s === 'failed' || s === 'inactive') return 'var(--red)'
  return 'var(--text-dim)'
}

function diskStatusColor(status: string) {
  const s = (status || '').toUpperCase()
  if (s === 'GOOD' || s === 'NORMAL') return 'var(--green)'
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: size }}>
      <svg width={w} height={h} style={{ overflow: 'visible' }}>
        <defs>
          <marker id="qnapArrowG" markerWidth="5" markerHeight="5" refX="2.5" refY="2.5" orient="auto">
            <path d="M0,0 L5,2.5 L0,5 Z" fill="var(--green)" />
          </marker>
          <marker id="qnapArrowA" markerWidth="5" markerHeight="5" refX="2.5" refY="2.5" orient="auto">
            <path d="M0,0 L5,2.5 L0,5 Z" fill="var(--amber)" />
          </marker>
        </defs>
        <line x1={ex1} y1={ey1} x2={ex2} y2={ey2} stroke="var(--green)" strokeWidth={2.5} strokeLinecap="round" markerEnd="url(#qnapArrowG)" opacity={0.85} />
        <line x1={ix1} y1={iy1} x2={ix2} y2={iy2} stroke="var(--amber)" strokeWidth={2.5} strokeLinecap="round" markerEnd="url(#qnapArrowA)" opacity={0.85} />
...
export default function QNAPPanel({ panel, heightUnits }: { panel: Panel; heightUnits: number }) {
  const [data, setData] = useState<QNAPData | null>(null)
  ...
  const sseData = useSSE<QNAPData>(integrationId)
  useEffect(() => {
  ...
  const rxMbs = data.netRxMbs || 0
  const txMbs = data.netTxMbs || 0

  const degradedVolumes = volumes.filter(v => {
    const s = (v.status || '').toLowerCase()
    return s && s !== 'normal' && s !== 'ready' && s !== 'good'
  })
  const warnDisks = disks.filter(d => {
    const s = (d.status || '').toUpperCase()
    return s === 'WARNING' || s === 'FAILED' || s === 'ERROR'
  })
        ...
        <div key={i}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 3 }}>
            ...
            <span style={{ fontSize: 12, fontWeight: 500, flex: 1, overflow: 'hidden',
              textOverflow: 'ellipsis', whiteSpace: 'nowrap' }} title={v.label}>
              {v.label}
            </span>
            {v.raidType && (
            ...
            {row.map((d, di) => (
              <div key={di} style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 4,
                ...
                <span style={{ width: 5, height: 5, borderRadius: '50%', flexShrink: 0,
                  background: diskStatusColor(d.status) }} />
                <span style={{ color: 'var(--text-dim)', flexShrink: 0, fontSize: 9 }}>{d.id}</span>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
      {disks.map((d, i) => (
        <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 6,
          ...
          <span style={{ width: 6, height: 6, borderRadius: '50%', flexShrink: 0,
            background: diskStatusColor(d.status) }} />
          <span style={{ color: 'var(--text-dim)', fontFamily: 'DM Mono, monospace', flexShrink: 0, minWidth: 32, fontSize: 10 }}>
            {d.id}
          </span>
          ...
          {d.smartStatus && d.smartStatus.toUpperCase() !== 'PASS' && d.smartStatus !== '' && (
            <span style={{ fontSize: 9, color: 'var(--amber)', fontFamily: 'DM Mono, monospace', flexShrink: 0 }}>
              {d.smartStatus}
            </span>
          )}
        </div>
  ...
  if (heightUnits < 4) return (
    <div style={{ height: '100%', overflow: 'auto' }}>
      ...
      {(rxMbs > 0 || txMbs > 0) && (
        <ArcRow>
          <NetWidget rxMbs={rxMbs} txMbs={txMbs} />
        </ArcRow>
      )}
