...
interface KeycloakFailure { username: string; clientIp: string; createdAt: string }
interface KeycloakData {
  uiUrl: string; days: number
  logins: number; failures: number; activeSessions: number
  recentFailures: KeycloakFailure[]
}
...
export default function KeycloakPanel({ panel, heightUnits }: { panel: Panel; heightUnits: number }) {
  const [data, setData] = useState<KeycloakData | null>(null)
  ...
  const sseSignal = useSSE<KeycloakData>(integrationId)
  useEffect(() => {
  ...
  // Keycloak's admin console URL scheme differs across major versions
  // (legacy AngularJS vs. the newer React console), so unlike Authentik
  // there's no stable deep link to a filtered events list — pills link to
  // the plain server root instead.
