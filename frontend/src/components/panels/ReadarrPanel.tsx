...
interface ReadarrData {
  uiUrl: string
  upcoming: ReadarrBook[]; history: ReadarrBook[]; missing: ReadarrBook[]
  bookCount: number; onDiskCount: number
}
...
function BookLink({ uiUrl, authorName, title }: { uiUrl: string; authorName: string; title: string }) {
  const href = uiUrl
    ? `${uiUrl}/author`
  ...
  return (
    <span>
      ...
      {authorName && title && <span style={{ color: 'var(--text-dim)', margin: '0 4px' }}>—</span>}
...
export default function ReadarrPanel({ panel, heightUnits }: { panel: Panel; heightUnits: number }) {
  ...
  const statsBar = (
    <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
      {[
        ...
        { label: 'Wanted',  value: (data.missing || []).length },
      ].map(s => (
  ...
  const BookRow = ({ b, showDate }: { b: ReadarrBook; showDate?: boolean }) => {
    const date = b.date || b.releaseDate || ''
    return (
      <div style={{ display: 'flex', alignItems: 'center', gap: 8,
        padding: '3px 0', borderBottom: '1px solid var(--border)', fontSize: 12 }}>
        <span style={{ flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
          <BookLink uiUrl={uiUrl} authorName={b.authorName} title={b.title} />
        </span>
        {showDate && date && (
  ...
  if (heightUnits < 3) return (
    <div style={{ height: '100%', overflow: 'auto' }}>
      ...
      {(data.history || []).length === 0
        ...
        : (data.history || []).map((b, i) => <BookRow key={i} b={b} showDate />)
      }
    </div>
  )
    ...
    <div style={{ height: '100%', overflow: 'auto' }}>
      ...
      {(data.upcoming || []).length > 0 && (
        <>
          ...
          {data.upcoming.map((b, i) => <BookRow key={i} b={b} showDate />)}
        </>
      )}
      {sectionTitle('Recently downloaded')}
      {(data.history || []).length === 0
      ...
      {heightUnits >= 4 && (data.missing || []).length > 0 && (
        <>
          ...
          {data.missing.map((b, i) => <BookRow key={i} b={b} showDate />)}
        </>
      )}
    </div>
  )
