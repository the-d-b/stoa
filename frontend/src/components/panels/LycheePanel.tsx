...
interface LycheePhoto { id: string; title: string; thumbUrl: string }
interface LycheeData {
  uiUrl: string; integrationId: string
  photos: number; albums: number; users: number; storage: number
  preview?: LycheePhoto[]
}
...
function PhotoCarousel({ photos, integrationId, uiUrl }: {
  photos: LycheePhoto[]; integrationId: string; uiUrl: string
}) {
  ...
  const thumbUrl = `/api/lychee/${integrationId}/thumb?src=${encodeURIComponent(photo.thumbUrl)}`
...
function StatGrid({ data, uiUrl }: { data: LycheeData; uiUrl: string }) {
  const stats = [
    { key: 'photos',  label: 'photos',  value: data.photos,  icon: '📷', href: uiUrl || undefined },
    { key: 'albums',  label: 'albums',  value: data.albums,  icon: '🗂️' },
    ...(data.storage > 0 ? [{ key: 'storage', label: 'storage', value: fmtBytes(data.storage), icon: '💾' }] : []),
...
export default function LycheePanel({ panel, heightUnits }: { panel: Panel; heightUnits: number }) {
  const [data, setData] = useState<LycheeData | null>(null)
  ...
  const sseData = useSSE<LycheeData>(integrationId)
  ...
  if (heightUnits <= 1) return (
    <div style={{ height: '100%', display: 'flex', alignItems: 'center', gap: 6 }}>
      ...
      <Stat label="albums"  value={data.albums} icon="🗂️" grow />
      {data.storage > 0 && <Stat label="storage" value={fmtBytes(data.storage)} icon="💾" grow />}
    </div>
  )
