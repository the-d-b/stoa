...
> **Your Stoa URL must be a real, publicly-resolvable domain.** Google rejects non-ICANN TLDs (`.home`, `.local`, `.lan`, and similar) as an OAuth redirect host — attempting to connect from a dashboard reachable only at one of these will fail with "Error 400: invalid_request — doesn't comply with Google's OAuth 2.0 policy," regardless of whether the redirect URI is otherwise registered correctly. Nothing needs to be internet-reachable at connect time beyond the callback redirect itself, but the hostname does need to resolve as a real domain — point a domain you own at your dashboard (even just for this) if it normally lives at a `.home`-style address.
