...
| autobrr | 🔶 Need Testing | Plain API key — autobrr → Settings → API | Required | 30 s | [autobrr/](autobrr/) |
...
| Actual Budget | 🔶 Need Testing | API key — set as `API_KEY` env var on the `actual-http-api` sidecar | Required | 5 min | [actualbudget/](actualbudget/) |
