...
- **API calls per poll:** 1 call to `/api/collections` for collection metadata, then 1 call per collection (up to 300 items) to `/api/collections/media/{id}/content/1` for poster images — typically 3–6 calls total depending on how many collections you have. Collections larger than 300 items get their posters on a slower, cached cycle (once every 24h) instead of every poll — that endpoint sorts the whole collection before paginating, so on very large collections it's expensive enough that paying it every 5 minutes would back up subsequent polls. Posters for these collections may be up to a day stale as a result
