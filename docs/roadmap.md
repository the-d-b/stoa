...
**Easier** (probably just need an instance spun up): NextDNS, WG-Easy, Home Assistant, Keycloak

**Done:** Pi-hole (0.17.9–0.17.10 — v6 API overhaul, time-range picker, blocklists), Homebox (0.17.11 — API token auth, By Tag, photo strip, deep links), Monica (0.17.12 — reminders field/date fix, error logging, Calendar panel source, found and reported an upstream Monica 4.1.2 bug where built-in life event types silently can't create reminders), FitTrackee (0.17.13 — fixed a multi-day duration display bug, expanded sport icon coverage, added a live 7d/30d/90d/All time-range pill selector), Firefly III (0.17.15 — fixed a wrong summary-field key that mislabeled and mis-sorted one figure, redesigned the panel to match Actual Budget's layout with a live month navigator, clarified the OAuth-vs-Personal-Access-Token confusion in setup docs), Traefik (0.17.16 — fixed a two-column 4x+ layout to stack vertically, matching Stoa's other panels), Wger (0.17.17 — fixed a silent weight-parsing bug, a session date field that had drifted from wger's actual released API vs its unreleased master branch, and a wrong 5-value impression scale; added a live 7d/30d/60d/90d/All time-range picker with a start→now weight chart; unit now mirrors the account's own kg/lb setting instead of forcing one), OpenMediaVault (0.17.18 — the roughest one yet: fixed a login field mismatch that made authentication always silently fail, several byte-count fields returned as quoted strings, a CPU/memory utilization scale mismatch producing 900%+ readings, and a pre-formatted filesystem-usage string; added RAID array status, per-disk SMART health, and a synthesized Alerts section since OMV has no unified alerts API of its own; confirmed disk I/O throughput isn't available as clean data at all)

**Harder** (need something already in place to test against): Unraid, Synology DSM, pfSense, Cloudflare, Paperless, Docspell, Strava, Twitch, Netbird, RomM, Pterodactyl, OpenWrt
