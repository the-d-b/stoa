...
- Rough shape: (1) a straightforward Portainer integration, (2) config options in Stoa describing what a "stack" of shared-library services looks like, (3) a UI that pushes the deployment through the Portainer integration. Believed to be less work than it sounds — smaller than the "new feature category" framing might imply. Still a genuine step up in trust level from anything Stoa does today (deploying infrastructure, not just reading or nudging it via an existing app's own write API like Radarr's) — sequence after the holistic security review (§5), given the review matters more once Stoa can create infrastructure, not just call other apps' write endpoints.
